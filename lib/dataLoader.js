  ...
  return this.fetchFiles(keys)
      ...
      permissions = txData.reduce(function(memo, txData, i) {
        ...
        switch (txData.type()) {
          case TransactionData.types.PUBLIC: // we already have our file
            ...
            self.emit('file', files[i], keys[i]);
            self.emit('file:public', files[i], keys[i]);
          ...
          case TransactionData.types.CLEARTEXT_STORE:
            ...
            self.emit('file', body, keys[i]);
            self.emit('file:permission', body, keys[i]);
    ...
    .then(function(sharedFiles) {
      ...
      sharedFiles.forEach(function(file, idx) {
        ...
        self.emit('file:shared', file, keys[fileIdx]);
        files[fileIdx] = file;
      });
...
DataLoader.prototype.saveIfNew = function(file, tx) {
  var self = this;
