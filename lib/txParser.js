...
TxParser.prototype.parse = function(tx) {
  ...
  var sharedKey;
  if (this.addressBook) {
    from = find(tx.ins, function(i) {
      var addr = common.getAddressFromInput(i, this.networkName);
      return this.addressBook.byAddress(addr);
    }, this);

    to = find(tx.outs, function(o) {
  ...
  if (me && from && to) {
    priv = from.keys(from === me).identity[0];
    pub = to.keys(to === me).identity[0];
  }
  else {
    // priv = ...
    // pub = ...
