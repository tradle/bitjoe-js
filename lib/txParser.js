...
TxParser.prototype.parse = function(tx) {
  ...
  var pub;
  var sharedKey;
  var txInfo = getTxInfo(tx);
  if (this.addressBook) {
    from = find(txInfo.addresses.from, function(addr) {
      return this.addressBook.byFingerprint(addr)
    }, this)

    to = find(txInfo.addresses.to, function(addr) {
      return this.addressBook.byFingerprint(addr)
    }, this)
  }
...
function getTxInfo(tx) {
  ...
  addresses.from = tx.ins.map(function(i) {
    return common.getAddressFromInput(i, this.networkName);
  });

  addresses.to = tx.outs.map(function(o) {
    return common.getAddressFromOutput(o, this.networkName);
  });
